 ...
 */
class BMDriverr extends TestDriver implements GlobalConst {
  ...
  /**
   * BMDriverr Constructor, inherited from TestDriver
   */
  public BMDriverr () {
    super("buftest");
  }
  ...
  protected boolean test4 () {
		...
		int pagenumber=-1;
	        ...
	        try
	        {
	        	...
	        	if(replacer.getLast(pagenumber )>0)
	        	{
	      ...
	      if (status == OK)
	      {
	    	  try
	    	  {
	    		  ...
	    		  {
	    			  pagenumber=frames[i];
	    			 if(replacer.getHist(pagenumber,1)>System.currentTimeMillis())
	    			  {
	    				  status=FAIL;
	    			  }
